...
- `Scan` runs parallel visible `QWebEngine` workers and stores each completed profile in MySQL as it is scanned (`artbrowser.twitter_profiles` by default).
- The scan view shows a live SQL dashboard with the latest saved profiles, category counts, descriptions, and URLs.
